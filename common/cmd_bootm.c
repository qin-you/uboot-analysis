...
#ifdef CONFIG_BOOTM_LINUX
	[IH_OS_LINUX] = do_bootm_linux,		/*arch/arm/lib/bootm.c*/
#endif
#ifdef CONFIG_BOOTM_NETBSD
...
static int bootm_start(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	images.verify = getenv_yesno("verify");		/* "verify" defined in s5p_goni.h */
	...
	os_hdr = boot_get_kernel(cmdtp, flag, argc, argv,		/* os_hdr == 0x30008000, uimage addr */
	...
	if (images.os.type == IH_TYPE_KERNEL_NOLOAD) {		/*false*/
	...
	if (((images.os.type == IH_TYPE_KERNEL) ||
	    ...
	    (images.os.os == IH_OS_LINUX)) {			/* true*/
		/* find ramdisk */
		ret = boot_get_ramdisk(argc, argv, &images, IH_INITRD_ARCH,
				&images.rd_start, &images.rd_end);		/* if not use ramdisk return 1*/
		if (ret) {
...
static int bootm_load_os(image_info_t os, ulong *load_end, int boot_progress)
{
	...
	flush_cache(load, (*load_end - load) * sizeof(ulong));	/* flush uboot data in cache, kernel is coming */
...
int do_bootm(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (argc > 1) {
		...
		simple_strtoul(argv[1], &endp, 16);	/* normal case: endp == NULL */
	...
	if (bootm_start(cmdtp, flag, argc, argv))	/* here we go in right case. return 0 in right case */
		return 1;
	...
	ret = bootm_load_os(images.os, &load_end, 1);	/* flush cache. return 0 in right case */
	...
	lmb_reserve(&images.lmb, images.os.load, (load_end - images.os.load));	/* nothing */

	if (images.os.type == IH_TYPE_STANDALONE) {		/* false*/
		if (iflag)
			enable_interrupts();
	...
	boot_fn = boot_os[images.os.os];		/*IH_OS_LINUX*/
	...
	boot_fn(0, argc, argv, &images);		/* enter kernel */
		...
		ulong *os_len)
{
	...
	img_addr = genimg_get_image(img_addr);		/* do nothing */
	...
	*os_data = *os_len = 0;
	...
	case IMAGE_FORMAT_LEGACY:
		printf("## Booting kernel from Legacy Image at %08lx ...\n",
				img_addr);
		hdr = image_get_kernel(img_addr, images->verify);	/* print image header. hdr==img_addr */
		if (!hdr)
			return NULL;
		bootstage_mark(BOOTSTAGE_ID_CHECK_IMAGETYPE);

		/* get os_data and os_len */
		switch (image_get_type(hdr)) {		/* will get 0x02. use hexdump uImage and image_header_t */
		case IH_TYPE_KERNEL:
		case IH_TYPE_KERNEL_NOLOAD:
			*os_data = image_get_data(hdr);	    /* get true kernel addr(0x30008040) jumping header */
			*os_len = image_get_data_size(hdr);	/* get ih_size from image header. size of zImage */
			break;
		case IH_TYPE_MULTI:
